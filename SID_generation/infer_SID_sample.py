...
INPUT_EMBEDDING_COL = 'feature'  # 输入文件中embedding列的名称，例如"feature"
INPUT_ID_COL = 'base62_string'  # 输入文件中ID列的名称，例如"base62_string"
CKPT_PATH = './result/sample_data_bs2048_lr0.002_ep25_20251105_163419/checkpoint-24.pth'  # todo: 需要推理的ckpt
INPUT_FILE_PATH = './datas/final_feature/part_0.csv'  # todo: 输入的emb，可从https://huggingface.co/datasets/AL-GR/Item-EMB获取
OUTPUT_FILE_PATH = './result/inference_results_batch.csv'  # todo: 输出结果
...
def predict_batch(
    ...
    for item_id, embedding_str in zip(item_ids_batch, embeddings_batch):
        try:
            embedding_np = np.frombuffer(base64.b64decode(embedding_str), dtype=np.float32).reshape(-1)
